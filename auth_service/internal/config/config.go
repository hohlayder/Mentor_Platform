...
func InitConfig(configPath string) (*Config, error) {
	...
	if cfg.AccessSecret = os.Getenv("JWT_SECRET"); cfg.AccessSecret == "" {
...
func (tc *TokenConfig) Validate() error {
	...
	if tc.AccessExpiry == 0 {
		return fmt.Errorf("access expiry is required")
	}
	if tc.RefreshExpiry == 0 {
		return fmt.Errorf("refresh expiry is required")
	}
	if tc.AccessExpiry >= tc.RefreshExpiry {
